...
class Main extends PluginBase implements Listener {
    ...
    public function onEnable(): void{
        ...
        $this->saveResource('Maps/smallant.zip');
            ...
            if($choice == 1){
                if($this->arenas != null){
                    foreach ($this->arenas as $arena) {
                        $choice2 = $arena->choice;
                        if($choice2 == 1) {
                            if ($arena->players < 4) {
                                $this->getServer()->getWorldManager()->loadWorld($nick, true);
                                $position = new Position(256, 117, 256, $arena->world);
                                $arena->world->loadChunk(256, 256);
                                ...
                                while ($tag <= 12) {
                                    ...
                                    if (!is_dir($directory)) {
                                        $src = $this->getDataFolder() . "/Maps/smallant.zip";
                                        $zip = new \ZipArchive;
                                        $zip->open($src);
                                        $zip->extractTo($this->getServer()->getDataPath() . "worlds/");
                                        rename($this->getServer()->getDataPath() . "worlds/smallant", $this->getServer()->getDataPath() . "worlds/" . $dir_name);
                                        $this->getServer()->getWorldManager()->loadWorld($dir_name, true);
                                        $world = $this->getServer()->getWorldManager()->getWorldByName($dir_name);
                                        $player->sendMessage("[ANTWARS]>>>Подготовка мира...");
                                        $this->MapFiller($world, $size, $choice, $player);
                                        return true;
                                    } else {
                                        $tag++;
                                    }
                ...
                if($this->arenas != null) {
                    foreach ($this->arenas as $arena) {
                        if ($arena->choice == 3) {
                            if ($arena->players < 4) {
                                $this->getServer()->getWorldManager()->loadWorld($nick, true);
                                $position = new Position(256, 127, 256, $arena->world);
                                $arena->world->loadChunk(256, 256);
                                $player->teleport($position);
                                return true;
                            } else {
    ...
    public function MapFiller(World $world,int $size,$choice,$player){
        $factor = $size/2;
        $x_size1 = 256 - $factor;
        $x_size2 = 256 + $factor;
        $y_size1 = 65 - $factor;
        ...
        if($size === self::SMALL_ARENA_SIZE){
            ...
            if($choice == 1) {
                $a = new Arena($this, $world, $choice);
                array_push($this->arenas, $a);
                $a->onJoin($player);
            } else {
                $a = new Arena($this, $world, $choice);
                array_push($this->arenas, $a);
